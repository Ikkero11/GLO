"""
bot.py — Анти-стресс v0.5.0
Изменения v0.5.0:
  - Исправлена нумерация вопросов в экспресс-тесте (1/4 вместо 1/8)
  - Добавлена возможность скрывать/показывать кнопки главного меню (/menu_hide, /menu_show)
  - Добавлена ежедневная цитата психолога в 16:30
  - Полное удаление Markdown из всех мест где он вызывал ошибки
  - Исправлена команда /admin_users: кликабельные юзернеймы через mention
...
EXPRESS_QUESTIONS_POOL = [
    # Блок 1 — Телесное напряжение
    [
        "🙆 *Вопрос 1 из 4 — Тело*\n\nНасколько ты чувствуешь напряжение в теле прямо сейчас?\n\n_1 — совсем нет  |  5 — очень сильное_",
        "🙆 *Вопрос 1 из 4 — Тело*\n\nЕсть ли зажатость в плечах, шее или спине сегодня?\n\n_1 — совсем нет  |  5 — очень сильная_",
        "🙆 *Вопрос 1 из 4 — Тело*\n\nКак бы ты оценил физическое напряжение в теле за сегодня?\n\n_1 — полностью расслаблен  |  5 — очень напряжён_",
    ...
    # Блок 2 — Цифровой шум
    [
        "📱 *Вопрос 2 из 4 — Цифровой шум*\n\nНасколько тебя утомляют уведомления, новости, соцсети сегодня?\n\n_1 — совсем нет  |  5 — очень сильно_",
        "📱 *Вопрос 2 из 4 — Цифровой шум*\n\nСколько раз сегодня ты бездумно тянулся к телефону?\n\n_1 — почти не тянулся  |  5 — постоянно_",
        "📱 *Вопрос 2 из 4 — Цифровой шум*\n\nЧувствуешь ли перегрузку от потока информации сегодня?\n\n_1 — совсем нет  |  5 — сильная перегрузка_",
    ],
    # Блок 3 — Навязчивые мысли
    [
        "🌀 *Вопрос 3 из 4 — Мысли*\n\nКак часто сегодня крутились одни и те же тревожные мысли?\n\n_1 — совсем нет  |  5 — постоянно_",
        "🌀 *Вопрос 3 из 4 — Мысли*\n\nТяжело ли было «отключить голову» сегодня?\n\n_1 — легко  |  5 — очень тяжело_",
        "🌀 *Вопрос 3 из 4 — Мысли*\n\nВозникало ли ощущение, что мысли «идут по кругу»?\n\n_1 — совсем нет  |  5 — почти всё время_",
...
ADMIN_HELP_TEXT = (
    f"🛠 Админ-панель — {BOT_NAME} v{VERSION}\n\n"
    "Команды статистики:\n"
...
def moon_phase_key():
    """
    Точный расчёт фазы луны через библиотеку ephem.
    Определяет освещённость диска и направление (растёт/убывает).
    """
    import ephem
    moon = ephem.Moon()
    moon.compute(date.today().strftime("%Y/%m/%d"))
    illum = moon.phase  # освещённость диска в процентах (0–100)

    # Определяем растёт или убывает луна:
    # сравниваем освещённость сегодня и завтра
...
async def menu_moon(msg: Message):
    ...
    try:
        if photo:
            await msg.answer_photo(FSInputFile(photo), caption=caption)
        else:
...
async def cmd_admin_users(msg: Message):
    ...
    for u in users:
        ...
        # Кликабельный mention через HTML
